...
import logo from "../assets/logo/SSRU-Logo.jpg";
    ...
    <nav className="bg-white border-b border-gray-200 shadow-md sticky top-0 z-50">
        ...
        <div className="flex justify-between h-24 items-center">
          ...
          <div className="sm:hidden">
            <button
              ...
              className="text-[#5ab1cf] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#5ab1cf]"
              ...
              {links.map((link) => {
                ...
                return (
                  <li key={link.name} className="flex-shrink-0">
                    <Link
                      to={link.path}
                      className={`block py-2 px-3 text-[#5ab1cf] text-lg font-medium relative
                        after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[2px] after:bg-[#5ab1cf] after:transition-all
                        hover:after:w-full
      ...
      >
        <div className="px-4 sm:px-6">
          <ul className="mt-2 mb-4 space-y-1 rounded-lg bg-white p-2 shadow-md">
            {links.map((link) => {
              const isActive = location.pathname === link.path;
              return (
                <li key={link.name}>
                  <Link
                    to={link.path}
                    className={`block rounded-md py-3 px-4 text-[#5ab1cf] text-lg font-medium ${
                      isActive ? "bg-[#5ab1cf]/10" : "hover:bg-[#5ab1cf]/10"
                    }`}
                    onClick={() => setIsOpen(false)}
                  >
                    {link.name}
